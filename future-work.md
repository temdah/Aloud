...
- **Compress cached audio** *(in progress — local Expo module, libvorbis JNI)* — the cache stores raw WAV (`wavEncoder` in `src/supertonic/synthesis/wavEncoder.ts`), which is large (~172 KB/s of audio) and slow to write. Encoding to Ogg Vorbis would cut cache size ~10× and speed up file IO. **Codec note:** the model outputs 44.1 kHz, which Opus can't encode without resampling — Vorbis encodes 44.1 kHz natively, so we chose it (no resampler needed). There is no pure-JS Vorbis encoder we can ship — encoding requires a native module (a `libvorbis` JNI bridge in a local Expo module), which forces a native rebuild (`expo prebuild`). Doing the WAV→PCM→Vorbis encode in JS would be far too slow on-device. **Concrete path when picked up:** (a) add a native encoder module behind a small `encodeAudio(pcm, sampleRate) → Uint8Array` interface; (b) swap `wavEncoder` for it in `audioCache`, keeping WAV as a fallback when the native module is absent; (c) confirm expo-audio can play the chosen container (`.ogg`/`.m4a`) via `player.replace`; (d) bump a cache-version key so old WAV caches are re-rendered. **Touches:** new native module, `narration/audioCache.ts`, `wavEncoder.ts`, manifest cache-version.
- **Async cache writes** — ensure WAV/Vorbis writes don't block the synthesis loop; write in the background and update the manifest atomically.
...
- **Export audiobook** — once "make full audiobook" exists, allow exporting the rendered audio as a single `.m4b`/`.ogg` file to share or sideload.
