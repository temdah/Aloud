...
// (documentDirectory/images/<docHash>/<id>.jpg). The extraction WebView can't
// write files, so it sends base64; we decode + persist here and the block keeps
// only the file uri (cache stays small).
...
// Decode base64 to bytes — RN has no guaranteed atob.
