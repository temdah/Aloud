...
// Loads the config, tokenizer, and four ONNX sessions from local storage and
// assembles a ready-to-use TextToSpeech instance. Assumes files are downloaded.
export async function loadTextToSpeech(): Promise<TextToSpeech> {
  const config = (await modelFile(CONFIG_FILE).json()) as SupertonicConfig;
  const indexer = (await modelFile(INDEXER_FILE).json()) as number[];
  const textProcessor = new UnicodeTextProcessor(indexer);

  const create = (name: string) => InferenceSession.create(ortModelPath(name), SESSION_OPTIONS);
...
export async function loadVoiceStyle(voice: string): Promise<VoiceStyle> {
  const data = (await modelFile(voiceFileName(voice)).json()) as VoiceStyleData;
